...
import {createProgramm} from "store/slices/programmsSlice.ts";
import {T_ProgrammAddData} from "modules/types.ts";

const ProgrammAddPage = () => {
    ...
    const handleCreateProgramm = async() => {
        ...
        formData.append('material', material as string)

        if (imgFile != undefined) {
            formData.append('image', imgFile, imgFile.name)
        }

        await dispatch(createProgramm(formData as T_ProgrammAddData))
            ...
            <Row>
                ...
                <Col md={6}>
                    ...
                    <Col className="d-flex justify-content-center gap-5 mt-5">
                        <Button color="success" className="fs-4" onClick={handleCreateProgramm}>Создать</Button>
                    </Col>
                </Col>
            </Row>
        </Container>
