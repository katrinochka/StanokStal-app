...
const ManufacturesPage = () => {
    ...
    useEffect(() => {
        const intervalId = setInterval(handleFetchManufactures, 2000)
        return () => clearInterval(intervalId)
    }, [filters]);

    const handleFetchManufactures = () => dispatch(fetchManufactures())
    ...
    return (
        <Container>
            ...
            {manufactures.length ? <ManufacturesTable manufactures={manufactures} /> : <h3 className="text-center mt-5">Детали не найдены</h3>}
        </Container>
    )
};
