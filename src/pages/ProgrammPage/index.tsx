import * as React from 'react';
import {useParams} from "react-router-dom";
import {useEffect} from "react";
import {T_Programm} from "src/modules/types.ts";
import {Col, Container, Row} from "reactstrap";
import {ProgrammMocks} from "src/modules/mocks.ts";
import mockImage from "assets/mock.png";

type Props = {
    selectedProgramm: T_Programm | null,
    setSelectedProgramm: React.Dispatch<React.SetStateAction<T_Programm | null>>,
    isMock: boolean,
    setIsMock: React.Dispatch<React.SetStateAction<boolean>>
}

const ProgrammPage = ({selectedProgramm, setSelectedProgramm, isMock, setIsMock}: Props) => {
    const { id } = useParams<{id: string}>();

    const fetchData = async () => {
        try {
            const response = await fetch(`/api/programms/${id}`)
            const data = await response.json()
            setSelectedProgramm(data)
        } catch {
            createMock()
        }
    }

    const createMock = () => {
        setIsMock(true)
        setSelectedProgramm(ProgrammMocks.find(programm => programm?.id == parseInt(id as string)) as T_Programm)
    }

    useEffect(() => {
        if (!isMock) {
            fetchData()
        } else {
            createMock()
        }

        return () => setSelectedProgramm(null)
    }, []);

    if (!selectedProgramm) {
        return (
            <div>

            </div>
        )
    }

    return (
        <Container>
            <Row>
                <Col md="6">
                    <img
                        alt=""
                        src={isMock ? mockImage as string : selectedProgramm.image}
                         className="programm-image w-100"
                    />
                </Col>
                <Col md="6" className="d-flex flex-md-column gap-md-3">
                   <div className="header-text" style={{ fontSize: '40px' }}>{selectedProgramm.name}</div>
                    <span>Описание: {selectedProgramm.description}</span>
                    <span>Цена: {selectedProgramm.price}₽</span>
                    <span>Материал: {selectedProgramm.material}</span>
                </Col>
            </Row>
        </Container>
    );
};

export default ProgrammPage