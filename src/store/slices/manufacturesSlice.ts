...
import {E_ManufactureStatus, T_Manufacture, T_ManufacturesFilters, T_Programm} from "modules/types.ts";
...
export const fetchManufactures = createAsyncThunk<T_Manufacture[], object, AsyncThunkConfig>(
    ...
    async function(_, thunkAPI) {
        ...
        return response.data.filter(manufacture => manufacture.owner.includes(state.manufactures.filters.owner))
    }
)
...
export const acceptManufacture = createAsyncThunk<void, string, AsyncThunkConfig>(
    "manufactures/accept_manufacture",
    async function(manufacture_id,{dispatch}) {
        await api.manufactures.manufacturesUpdateStatusAdminUpdate(manufacture_id, {status: E_ManufactureStatus.Completed})
        await dispatch(fetchManufactures)
    }
)

export const rejectManufacture = createAsyncThunk<void, string, AsyncThunkConfig>(
    "manufactures/accept_manufacture",
    async function(manufacture_id,{dispatch}) {
        await api.manufactures.manufacturesUpdateStatusAdminUpdate(manufacture_id, {status: E_ManufactureStatus.Rejected})
        await dispatch(fetchManufactures)
    }
)
